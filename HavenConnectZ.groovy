 ...
 */
metadata {
	definition(name: "Z-Wave Lock Without Codes", namespace: "hubitat", author: "Scottie Holden", runLocally: true, executeCommandsLocally: false, mnmn: "hubitat", vid: "generic-lock-2") {
		capability "Actuator"
		capability "Lock"
		capability "Refresh"
		capability "Sensor"
...
/**
 * Configures the device to settings needed by Hubitat at device discovery time
 */
def configure() {
...
def parse(String description) {
	...
	if (description.startsWith("Err")) {
		if (state.sec) {
			...
			result = createEvent(
				descriptionText: "This lock failed to complete the network security key exchange. If you are unable to control it via hubitat, you must remove it from your network and add it again.",
				eventType: "ALERT",
